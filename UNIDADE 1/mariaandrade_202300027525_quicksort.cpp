...
int main (int argc, char* argv[]){
    ...
    Copias* copias = new Copias[numVetores];
    ...
    for (int i = 0; i < numVetores; i++){
        ...
        for (int j = 0; j < leituraVetores[i].tamanho; j++){
            int num;
            arquivoIn >> num;
            leituraVetores[i].vetor[j] = num;
        };

        leituraQuicks[0].contador = leituraVetores[i].tamanho; 

        copias[i].LP = new int[leituraVetores[i].tamanho];
        copias[i].LM = new int[leituraVetores[i].tamanho];
        copias[i].LA = new int[leituraVetores[i].tamanho];
        copias[i].HP = new int[leituraVetores[i].tamanho];
        copias[i].HM = new int[leituraVetores[i].tamanho];
        copias[i].HA = new int[leituraVetores[i].tamanho];

        for (int x = 0; x < leituraVetores[i].tamanho; x++){
            copias[i].LP[x] = leituraVetores[i].vetor[x]; 
            copias[i].LM[x] = leituraVetores[i].vetor[x]; 
            copias[i].LA[x] = leituraVetores[i].vetor[x];
            copias[i].HP[x] = leituraVetores[i].vetor[x];
            copias[i].HM[x] = leituraVetores[i].vetor[x];
            copias[i].HA[x] = leituraVetores[i].vetor[x];
        };

        cont = 0;
        quickSort(copias[i].LP, 0, leituraVetores[i].tamanho - 1, "LP");  
        leituraQuicks[1].contador = cont;
        
        cont = 0;
        quickSort(copias[i].LM, 0, leituraVetores[i].tamanho - 1, "LM");  
        leituraQuicks[2].contador = cont;

        cont = 0; 
        quickSort(copias[i].LA, 0, leituraVetores[i].tamanho - 1, "LA");  
        leituraQuicks[3].contador = cont;

        cont = 0;  
        quickSort(copias[i].HP, 0, leituraVetores[i].tamanho - 1, "HP");
        leituraQuicks[4].contador = cont;  

        cont = 0; 
        quickSort(copias[i].HM, 0, leituraVetores[i].tamanho - 1, "HM");  
        leituraQuicks[5].contador = cont;
        
        cont = 0; 
        quickSort(copias[i].HA, 0, leituraVetores[i].tamanho - 1, "HA");  
